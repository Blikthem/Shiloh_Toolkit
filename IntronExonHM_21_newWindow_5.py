...
#Inputs
##------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
gtfFileName="/home/claypooldj/genomes/gencode_hg38.longestTranscript.csv"    #The intermediate gtf longest splice varient csv file (generated from a seperate script) to use for annotation. {Type - $
csvDir="/home/claypooldj/myPythonScripts/testBedHM/hInputs/"    #Tlhe directory containing the cluster csv files to analyze. All csv files in this directory will be tested (so make sure there aren't other$
outputDir="/home/claypooldj/myPythonScripts/testBedHM/output2/"    #The directory where you want to generate all output files. {Type - String}
...
wValuesByRC=False    #Whether or not the program should weight the impact of each indivudal gene on the overall metagene average linearly with the number of clusters that align to that gene {Type -$
randStates=[7211995,541995,3131994,111,222,333,444,555,888,999]   #The random states to use in the binning algorithm, which randomly deals with rounding error and then averages the results. Can be $
randStates=[541995]
dpi=250    #The resolution of the output graphs in dpi {Type  d- Int}
...
wtGE="/Volumes/Untitled/Output/explorador/RNA_Seq_Background/moreWTExpressions/geneExpressionMatrix_TPM_withGeneNames.csv"    #A csv file containing the wild type gene expression levels from RNA-Se$
wtGE=""
...
class anotGene:
    ...
    def sideOfAdjacentIntron(self,exon):
        ...
        Returns:
            Dict) - links a L or R tag with the row it aligns to
        """
        toRet={}
        intronBefore=False
        intronAfter=False
...
class clusterGene:
    ...
    def populateHLTotal(self):
        """
        Populate the list of values which represent the total number of hits in this gene. This value will be used to normalize the values of all individual distributions within a given gene so that they can be averaged to create gene-level average distributions (which are then averaged for the metagene)
        """
        toRet=0
        for subCat in [self.hitListsExon,self.hitListsIntron]:
            for indL in subCat:
               toRet=toRet+sum(indL)
        self.totalHitSum=toRet
    
    def populateHitLists(self,anotFile):
        """
        Populate the hit lists of the clusterGene. In otherwords, pass stepwise over each nucleotide in this gene and see if it is overlapped by a binding cluster (asign a value of 1) or not overlapped (assign a value of 0).
        ...
        #print(self.gene,"Exon start indices ",matchedMG.exonIndexStarts)
        #print(self.gene,"Exon stop indices ",matchedMG.exonIndexStops)
    ...
    def populateIntronBinsOfPercents(self,anotFile,numberOfBins,randomStatesList):
        ...
        if matchedGene==False or self.gene=="" or self.totalHitSum==0:
            return(False)
        ...
        for h in range(0,len(self.hitListsIntron)):
            ...
            for indivSeed in randomStatesList:      
                ...
                nucPerc = 100.0/self.totalHitSum
    ...
    def populateGeneDistributions(self,NIntronBins,doubleFlankReq,anotFile,nIntronScan):
        """
        Averages the percentage lists for each exon and its flanking intronic space in this gene in order to get the average intron/exon distribution of clusters across this gene.
        ...
        Input:
            NIntronBins (Int) - The number of intronic bins which should be considered
            doubleFlankReq (bool) - Whether an exon needs to have introns adjacent on both sides in order to be considered.
            anotFile (AnotationFile Object) - Annotation file object used for the annotation.
            nIntronScan (int) - The number of intronic bins to consider for the flanking distributions on either side of the exon.
        """
        ...
        prevIntronStart=len(self.lstOfIntronBinLists[0])-nIntronScan
        
        #Get the sorted map of exon adjacencies
        adjacentMap=[]
        matchedMG = self.getMatchingMeta(anotFile)
        for indRo in matchedMG.lstOfAnotRows:
            if indRo.ty=="exon":
                #Get the ID of adjacent introns
                adjSide=matchedMG.sideOfAdjacentIntron(indRo)
                adjacentMap.append(adjSide)
        
        #For each exon distribution
        listOfExonDictionaries=[]
        for e in range(0,len(self.lstOfExonBinLists)):
            #Initialize the dictionary of window values (before intron, exon, after intron)
            windowDict={}
            curDict=adjacentMap[e]
            windowDict["ExonDistribution"]=self.lstOfExonBinLists[e]
            #Get the previous intron list
            if curDict.get("L")==False or "L" not in curDict:
                windowDict["FIntronDistribution"]=[]
            else:
                refIntron=curDict.get("L")
                #Get the intron distribution
                iIndex=0
                for ro in matchedMG.lstOfAnotRows:
                    if ro.start==refIntron.start and ro.stop==refIntron.stop and ro.ty==refIntron.ty:
                        break
                    if ro.ty=="intron":
                        iIndex=iIndex+1
                #Get the associated (end) intron rows
                prevI=[]
                for j in range(prevIntronStart,len(self.lstOfIntronBinLists[0])):
                    prevI.append(self.lstOfIntronBinLists[iIndex][j])
                windowDict["FIntronDistribution"]=prevI
                
            #Get the next intron list
            if curDict.get("R")==False or "R" not in curDict:
                windowDict["TIntronDistribution"]=[]
            
            else:
                refIntron=curDict.get("R")
                #Get the intron distribution
                iIndex=0
                for ro in matchedMG.lstOfAnotRows:
                    if ro.start==refIntron.start and ro.stop==refIntron.stop and ro.ty==refIntron.ty:
                ...
                #Get the associated (end) intron rows
                nextI=[]
                theIntronList=self.lstOfIntronBinLists[iIndex]
                for j in range(0,nIntronScan):
                    nextI.append(theIntronList[j])
                windowDict["TIntronDistribution"]=nextI

            if "-" in matchedMG.lstOfAnotRows[0].orientation:
                newT=windowDict.get("FIntronDistribution")
                newT.reverse()
                newF=windowDict.get("TIntronDistribution")
                newF.reverse()
                windowDict["TIntronDistribution"]=newT
                windowDict["FIntronDistribution"]=newF
            
            listOfExonDictionaries.append(windowDict)
            
        #Average the window dictionaries
        FI_toAvg=[]
        TI_toAvg=[]
        E_toAvg=[]
        for indDict in listOfExonDictionaries:
            if indDict["TIntronDistribution"]==[] or windowDict["FIntronDistribution"]:
                if doubleFlankReq==True:
                    continue
            
            E_toAvg.append(indDict["ExonDistribution"])
            if indDict["FIntronDistribution"]!=[]:
                FI_toAvg.append(indDict["FIntronDistribution"])
            
            if indDict["TIntronDistribution"]!=[]:
                TI_toAvg.append(indDict["TIntronDistribution"])
            
            E_toAvg.append(indDict["ExonDistribution"])
        
        FI_DF=pd.DataFrame(FI_toAvg)
        TI_DF=pd.DataFrame(TI_toAvg)
        E_DF=pd.DataFrame(E_toAvg)        
        
        #!!
        self.percentDE=list(E_DF.mean(axis = 0))
        self.percentDFI=list(FI_DF.mean(axis = 0))
        self.percentDTI=list(TI_DF.mean(axis = 0))
        
    
    def appendToLog(self,logN): 
        """
        Write the contents of the cluster gene to an append file.
        """
        f = open(logN, 'a')
        #Gene name
        if (sum(self.percentDFI)+sum(self.percentDE)+sum(self.percentDFI))!=0:
            f.write(self.gene)
            f.write("\n----------------------------------------")
            f.write("\n Hit lists-------")
            f.write(str(self.hitListsExon))
            f.write("\n")
            f.write(str(self.hitListsIntron))
            f.write("\n")
            f.write("Percentage bins-------")
            f.write(str(self.lstOfExonBinLists))
            f.write("\n")
            f.write(str(self.lstOfIntronBinLists))
            f.write("\n")
            f.write("\nFinal splice windows -------")
            f.write("\n")
            f.write(str(self.percentDFI))
            f.write("\n")
            f.write(str(self.percentDE))
            f.write("\n")
            f.write(str(self.percentDFI))
            f.write("\n .")
            f.close()
...
class parclip:
    ...
    def populateAllHits(self,anotF,NIntronBins,NExonBins,randomStatesList,nIntronScan,onlySandwitched):
        """
        ...
        Input:
            ...
            nIntronScan(int) - The number of bins within the intronic regions (up and down stream) that should be displayed along with the exon.
            onlySandwitched(bool) - Whether to only consider distribution curves from the exons (and their adjacent introns) which have introns on BOTH sides. Essentially, do we consider exons with only one adjacent intron.
        ...
        for chrom in self.lstOfChromosomesClustersGenes:
            ...
            for cGene in chrom.lstOfAnotElements:
                ...
                cGene.populateGeneDistributions(NIntronBins,onlySandwitched,anotF,nIntronScan)
        
    def getParclipPercentageDistributions(self,NIntronBins,NExonBins,clusterThreshold,weighValuesByRC,wtGE,FI_Dict,E_Dict,TI_Dict):
        """
        ...
        Input:
            ...
            FI_Dict (Dict) - The dictionary containing the gene maps (which genes are contributing density by bin) for the 5' Intron segment.
            E_Dict (Dict) - The dictionary containing the gene maps (which genes are contributing density by bin) for the exonic segment.
            TI_Dict (Dict) - The dictionary containing the gene maps (which genes are contributing density by bin) for the 3' Intron segment.
    
        """    
        #Remove all the genes that have fewer than the specified number of clusters set its lists equal to zero
        for chrom in self.lstOfChromosomesClustersGenes:
            #Get each gene
            for cGene in chrom.lstOfAnotElements:
        ...
        #Get the average distribution values
        for indDistribution in ["DFI","DTI","DE"]:
            toAvg=[]
            for chrom in self.lstOfChromosomesClustersGenes:
                #Get each gene
                for cGene in chrom.lstOfAnotElements:
                    #Add to the appropriate tracking dictionary to identify which genes are contributing density where.
                    if indDistribution=="DFI":
                        for i in range(0,len(cGene.percentDFI)):
                            theVal=cGene.percentDFI[i]
                            if theVal>0:
                                if i not in FI_Dict:
                                    FI_Dict[i]=[cGene.gene]
                                else:
                                    FI_Dict[i].append(cGene.gene)
                                
                    if indDistribution=="DTI":
                        for i in range(0,len(cGene.percentDTI)):
                            theVal=cGene.percentDTI[i]
                            if theVal>0:
                                if i not in TI_Dict:
                                    TI_Dict[i]=[cGene.gene]
                                else:
                                    TI_Dict[i].append(cGene.gene)
                    if indDistribution=="DE":
                        for i in range(0,len(cGene.percentDE)):
                            theVal=cGene.percentDE[i]
                            if theVal>0:
                                if i not in E_Dict:
                                    E_Dict[i]=[cGene.gene]
                                else:
                                    E_Dict[i].append(cGene.gene)
                                    
                    #Weight by read count
                    if weighValuesByRC==True:
                        for i in range(0,cGene.getTotalNumReads()):
                            if indDistribution=="DFI":
                                toAvg.append(cGene.percentDFI)
                            if indDistribution=="DTI":
                                toAvg.append(cGene.percentDTI)
                            if indDistribution=="DE":
                                toAvg.append(cGene.percentDE)
                    elif weighValuesByRC!=True:
                        if indDistribution=="DFI":
                            toAvg.append(cGene.percentDFI)
                        if indDistribution=="DTI":
                            toAvg.append(cGene.percentDTI)
                        if indDistribution=="DE":
                            toAvg.append(cGene.percentDE)
            #Get the average
            toAvgDF=pd.DataFrame(toAvg)
            theMean=list(toAvgDF.mean(axis = 0))
            
            #AVERAGE SAVING - REMOVER ME AFTER!!!!!
            #Save the 
            if indDistribution=="DFI":
                toAvgDF.to_csv("/home/claypooldj/myPythonScripts/nIE_DFI.csv")
            if indDistribution=="DTI":
                toAvgDF.to_csv("/home/claypooldj/myPythonScripts/nIE_DTI.csv")
            if indDistribution=="DE":
                toAvgDF.to_csv("/home/claypooldj/myPythonScripts/nIE_DE.csv")          
            #!!!!!
            
            if indDistribution=="DFI":
                self.ifDistribution=theMean
            if indDistribution=="DTI":
                self.itDistribution=theMean
            if indDistribution=="DE":
                self.eDistribution=theMean                                
 
        print("PAR-CLIP: ",self.filename)
        print("---------------- Final 5' Intron Section Distribution: ",self.ifDistribution)   
        print("---------------- Final Exon Distribution: ",self.eDistribution)
        print("---------------- Final 3' Intron Section Distribution: ",self.itDistribution) 
        print("AVG of 5' Intron",Average(self.ifDistribution))
        print("AVG of Exon",Average(self.eDistribution))
        print("AVG of 3' Intron",Average(self.itDistribution))
        print("Total sum: ",sum(self.eDistribution)+sum(self.itDistribution)+sum(self.ifDistribution))
...
##Functions
##------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
def writeGeneListDict(inpDict,outputDir,outTitle):
    """
    Write a dictionary of bin lists (links a bin number to the name of the genes that contribute density to the bin) to output file.
    Inputs:
        inpDict (dict) - The dictionary map.
        outputDir (str) - The directory where the dictionary should be saved.
        outTitle (Str) - The name to use in the output file.
    """
    toSave=outputDir+"/"+outTitle+"_GenesByBin.txt"
    file1 = open(toSave,"w+")  
    header=outTitle+": Genes By Bin"
    file1.write(header)
    file1.write("\n")
    #For each bin
    for binID in inpDict:
        #Write the bin name
        toW = "Bin # "+str(binID) +"\n"
        #Write the genes
        file1.write(toW)
...
def createExportTable(lstOfPCs,myDir,genesToUse,intronWind):
    """
    ...
    for pc in lstOfPCs:
        ...
        labelFI = pc.filename+": "+"5' Intron Section"
        labelE = pc.filename+": "+"Exon "
        labelTI = pc.filename+": "+"3' Intron Section"
            #Get the 5' Intron
        DEH1 = pc.ifDistribution
            #Add the label
        if labelFI not in DEH1:
            DEH1.insert(0,labelFI)
        ...
        DI.insert(0,labelE)
        toRetLst.append(DI)
            #Get the 3' half of the exon
        DEH2 = pc.itDistribution
            #Add the label
        if labelTI not in DEH2:
            DEH2.insert(0,labelTI)
    ...
    if len(genesToUse)==0:
        csvfile=csvfile+"/IntronExonTable_exonCentered.csv"
    elif len(genesToUse)!=0:
        csvfile=csvfile+"/"+genesStrToAdd+"IntronExonTable_exonCentered.csv"
...
def createSubPlots(exportTable,outputDir,NIntronBins,NExonBins,genesToUse,lstOfPCs,inpDPI,imgFormat,intronScan):
    """
    ...
    for w in indicesToCheck(exportTable):
        ...
        theDomain = []        
        for x in range(1,len(theRange)+1):
            theDomain.append(x)    
        ...
        if (indic==len(indicesToCheck(exportTable))):
            nAx.axes.text(-7, mVal*1.05, "5' Adj Intron", fontsize=10,color='b')
            nAx.axes.text(line1Val+.5*NExonBins, mVal*1.04, "Exon", fontsize=10,color='b')
            nAx.axes.text(line2Val, mVal*1.04, "3' Adj Intron", fontsize=10,color='b')
    
    os.chdir(outputDir)
    
    if len(genesToUse)==0:
        if imgFormat=="pdf":
            toSave='DistributionPlot_Intron_Exon_newWindow.pdf'
        if imgFormat!="pdf":
            toSave='DistributionPlot_Intron_Exon_newWindow.png'
    elif len(genesToUse)!=0:
        if imgFormat=="pdf":
            toSave=genesStrToAdd+"DistributionPlot_IntronExon_exonCentered.pdf"
        if imgFormat!="pdf":
            toSave=genesStrToAdd+"DistributionPlot_IntronExon_exonCentered.png"
...
def s_heatmap(pdDF,outDir,xAxisLab,genesToUse,inpDPI,imgFrmt):
    ...
    if len(genesToUse)==0:
        if imgFrmt=="pdf":
            toSave='DistributionHeatMap_Intron_Exon_exonCentered.pdf'
        elif imgFrmt=="png":
            toSave='DistributionHeatMap_Intron_Exon_exonCentered.png'
    elif len(genesToUse)!=0:
        if imgFrmt=="pdf":
            toSave=genesStrToAdd+"DistributionHeatMap_Intron_Exon_exonCentered.pdf"
        elif imgFrmt=="png":
            toSave='DistributionHeatMap_Intron_Exon_exonCentered.png'
    g.savefig(toSave, bbox_inches='tight',dpi=inpDPI)
        

def createHeatMapSingleton(lstOfParClips,outputDir,numIntronBins,numExonBins,genesToUse,inpDPI,imgFrmt,intronWindow):
    """
    ...
    xAxisLab=[]
    for x in range(0,numExonBins+intronWindow+intronWindow):
        if x==intronWindow:
            xAxisLab.append("-Exon Start")
        elif x==(intronWindow+numExonBins):
            xAxisLab.append("-Exon Stop")
        elif x!=intronWindow and x!=(intronWindow+numExonBins):
            xAxisLab.append("")
    ...
    for pc in lstOfParClips:
        ...
        ro=pc.ifDistribution+pc.eDistribution+pc.itDistribution
    ...
    #Creates the plot
    fig, ax = plt.subplots()
    im, cbar = heatmap(valueArray, yAxisLabels, xAxisLab, ax=ax,cmap="coolwarm", cbarlabel="Coverage")
    fig.tight_layout()
...
def createHeatMap(lstOfParClips,numIntronBins,numExonBins,outputDir,genesToUse,inpDPI,imgFrmt,intronWindow):
    """
...
##------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def generateIEVis(anotF,lstOfPCS):
    """
    """
    #For each PC
    for PC in lstOfPCS:
        #For each cluster gene
        for indChr in PC.lstOfChromosomesClustersGenes:
            for cGene in indChr.lstOfAnotElements:
                if len(cGene.lstOfExonBinLists)!=0:
                    print("Gene-----",cGene.gene)


##------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------         
def run(gtfFileName,csvDir,outputDir,NIntronBins,NExonBins,genesToUse,clusterThreshold,lowerBound,upperBound,boundFilter,weighValuesByRC,randomStatesLst,dpi,imgFormat,wildTypeGE,mainChromosomesOnly,intronWindow,flankedExonsOnly,bedAnotGTF,anoterScript):
    """
    ...
    Input:
        ...
        intronWindow(int) - The number of bins within the intronic regions (up and down stream) that should be displayed along with the exon.
        flankedExonsOnly(bool) - Whether to only consider distribution curves from the exons (and their adjacent introns) which have introns on BOTH sides. Essentially, do we consider exons with only one adjacent intron.
    ...
    Saves:
        export table - Binding distributions across regions in csv format.
        distribution curve - Individual binding curves represented as many plots in one figure.
        heat map - Heat map representation of the binding curves that has the rows clustered agglomeratively. 
    ...
    for exPC in lstOfParclips:
        exPC.populateAllHits(myGTFObj,NIntronBins,NExonBins,randomStatesLst,intronWindow,flankedExonsOnly)
        
        #Initialize the three dictionaries to store gene by bin information
        genesByBinDict_FI={}
        genesByBinDict_E={}
        genesByBinDict_TI={}
        exPC.getParclipPercentageDistributions(NIntronBins,NExonBins,clusterThreshold,weighValuesByRC,wtGE_File,genesByBinDict_FI,genesByBinDict_E,genesByBinDict_TI)
    
        writeGeneListDict(genesByBinDict_FI,outputDir,"FivePrimeAdjacentIntron")
        writeGeneListDict(genesByBinDict_E,outputDir,"Exon")
        writeGeneListDict(genesByBinDict_TI,outputDir,"ThreePrimeAdjacentIntron")

    print("...populated.")


    print("Creating the export table...")
    exportTable = createExportTable(lstOfParclips,outputDir,genesToUse,intronWindow) 
    print("...export table created")


    print("Creating the binding distribution curves for all parclips in the directory...")
    createSubPlots(exportTable,outputDir,NIntronBins,NExonBins,genesToUse,lstOfParclips,dpi,imgFormat,intronWindow)
    print("...binding distribution curves generated and merged.")
  
    print("Generating the heat map...")
    if len(lstOfParclips)>1:
        createHeatMap(lstOfParclips,NIntronBins,NExonBins,outputDir,genesToUse,dpi,imgFormat,intronWindow)
    else:
        createHeatMapSingleton(lstOfParclips,outputDir,NIntronBins,NExonBins,genesToUse,dpi,imgFormat,intronWindow)
    print("... heat map generated.")

##------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
##Run
##------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
#run(gtfFileName,csvDir,outputDir,1250,100,gtu,clustThreshold,lBound,uBound,boundF,wValuesByRC,randStates,dpi,imgFrmt,wtGE,mainChromosomes,intronScanRange,onlySandwitched,theBED_GTF,theBEDAnotScript)